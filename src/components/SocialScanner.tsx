    ...
    <div className="flex flex-col h-full bg-slate-950">
      <div className="p-6 border-b border-[#38bdf8]/20 bg-[#38bdf8]/5">
        <h2 className="text-xl font-bold flex items-center mb-2 tracking-tighter">
          <UserSearch className="w-5 h-5 mr-3 text-[#38bdf8]" /> USERNAME_CROSS_REF
        </h2>
        <p className="text-xs text-[#38bdf8]/60 uppercase tracking-widest">Search for identity availability and leaked accounts across social grids.</p>
      </div>
      ...
      <div className="p-6 flex-1 overflow-y-auto">
        ...
        {result && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h3 className="font-display text-[10px] font-bold text-[#38bdf8]/40 uppercase tracking-[0.3em] mb-4 border-b border-[#38bdf8]/10 pb-2 flex justify-between">
              ...
              {result.results.map((item: any, idx: number) => (
                <a 
                  ...
                  className={`p-4 border group transition-all duration-300 ${
                    item.found 
                      ? 'bg-[#38bdf8]/10 border-[#38bdf8]/40 hover:bg-[#38bdf8]/20' 
                      : 'bg-slate-950 border-[#38bdf8]/5 hover:border-[#38bdf8]/20 opacity-40 hover:opacity-100'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className={`text-xs font-bold ${item.found ? 'text-[#38bdf8]' : 'text-white/40'}`}>{item.name.toUpperCase()}</span>
                    <ExternalLink className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                  
                  <div className="mt-3 flex items-center justify-between">
                    {item.found ? (
                      <span className="text-[9px] font-bold text-[#38bdf8] bg-[#38bdf8]/10 px-2 py-0.5 border border-[#38bdf8]/20 tracking-tighter">DATA_MATCHED</span>
                    ) : (
                      <span className="text-[9px] font-bold text-white/20 tracking-tighter">
                        {item.status === 403 || item.status === 429 ? 'GRID_LOCKED' : 'NULL_DATA'}
                      </span>
                    )}
                    <span className="text-[8px] opacity-20 font-display">#{idx+1024}</span>
                  </div>
                </a>
              ))}
            </div>
